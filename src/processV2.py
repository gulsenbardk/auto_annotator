...
def getsingleXODR_asil(xodr_path, pcd_dir, output_dir):
    ...
    pcd_files = sorted(pcd_files)[:3]

    all_points = []
    for pcd_file in pcd_files:
        pcd_path = os.path.join(pcd_dir, pcd_file)
        points = las2array(pcd_path)
    ...
    original_pcd = np.vstack(all_points)
    ...
    #normal_points = np.array(gauss_pcd, dtype=np.float32)[:,:3]
    #normal_labels = np.array(gauss_lbl, dtype=np.int32)
    ...
    print("This code processing trainning dataset on 3 sampling type saving")
    ...
    save_icp_metrics_txt(
        filepath="/mnt/data/bard_gu/pcd/uniform_icp_metrics.txt",
    ...
    np.savetxt('/mnt/data/bard_gu/pcd/training_uniformV2.txt', data, fmt='%.6f %.6f %.6f %.6f %d')
    print("Uniform sampling save done!")
        
    """random_pcd, random_lbl, random_syn, random_rmse, random_fitness, random_class_rmse = icp_syn(original_pcd, random_points, random_labels, 2, 30, 0.2, 30, "point")
    save_icp_metrics_txt(
        filepath="/mnt/data/bard_gu/pcd/random_icp_metrics.txt",
        num_points=len(random_lbl),
        total_rmse=random_rmse,
        total_fitness=random_fitness,
        class_rmse=random_class_rmse
    )
    data = np.hstack((random_pcd, random_lbl.reshape(-1, 1)))
    np.savetxt('/mnt/data/bard_gu/pcd/training_random.txt', data, fmt='%.6f %.6f %.6f %.6f %d')
    print("Random sampling save done!")
        
    normal_pcd, normal_lbl, normal_syn, normal_rmse, normal_fitness, normal_class_rmse = icp_syn(original_pcd, normal_points, normal_labels, 2, 30, 0.2, 30, "point")
    save_icp_metrics_txt(
        filepath="/mnt/data/bard_gu/pcd/random_icp_metrics.txt",
        num_points=len(normal_lbl),
        total_rmse=normal_rmse,
        total_fitness=normal_fitness,
        class_rmse=normal_class_rmse
    )
    data = np.hstack((normal_pcd, normal_lbl.reshape(-1, 1)))
    np.savetxt('/mnt/data/bard_gu/pcd/training_normal.txt', data, fmt='%.6f %.6f %.6f %.6f %d')
    print("Normal sampling save done!")
